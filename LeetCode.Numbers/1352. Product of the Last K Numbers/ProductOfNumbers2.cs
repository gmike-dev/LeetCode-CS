...
public class ProductOfNumbers2
{
    ...
    {
        nums.Add(num);
        if ((nums.Count + BucketSize - 1) / BucketSize > buckets.Count)
            buckets.Add(num);
        else
            buckets[^1] *= num;
    }
    ...
    {
        var product = 1;
        var firstBucket = (nums.Count - k + BucketSize - 1) / BucketSize;
        for (var i = nums.Count - k; i < Math.Min(nums.Count, firstBucket * BucketSize); i++)
            product *= nums[i];
        for (var i = firstBucket; i < buckets.Count; i++)
            product *= buckets[i];
        return product;
    }
}
...
[TestFixture]
public class ProductOfNumbers2Tests
{
    [Test]
    public void Test1()
    {
        var productOfNumbers = new ProductOfNumbers2();
        ...
        productOfNumbers.GetProduct(3).Should()
            .Be(40); // return 40. The product of the last 3 numbers is 2 * 5 * 4 = 40
        productOfNumbers.GetProduct(4).Should()
            .Be(0); // return 0. The product of the last 4 numbers is 0 * 2 * 5 * 4 = 0
