...
public class SegmentTreeSolutionTests
{
    ...
    public void TestLargeInput(string nums, int k, int numOperations, int expected)
    {
        new SegmentTreeSolution().MaxFrequency(nums.Array(), k, numOperations).Should().Be(expected);
    }

    public static IEnumerable<object> GetTestCases()
    {
        string source = Path.Join(TestContext.CurrentContext.WorkDirectory,
            "3347. Maximum Frequency of an Element After Performing Operations II", "TestCases.txt");
