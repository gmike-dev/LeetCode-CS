namespace LeetCode.__Design._381._Insert_Delete_GetRandom_O_1____Duplicates_allowed.HashSet;
...
{
  private readonly Dictionary<int, HashSet<int>> indexes = new();
  private readonly List<int> items = new();

  public bool Insert(int val)
  {
    bool exists = indexes.TryGetValue(val, out var indexSet);
    if (exists)
    {
      indexSet.Add(items.Count);
    }
    else
    {
      indexes[val] = [items.Count];
    }
    items.Add(val);
    return !exists;
  }
  ...
  {
    if (!indexes.TryGetValue(val, out var indexSet))
    {
      return false;
    }

    int valIndex = indexSet.First();
    ...
    {
      items[valIndex] = lastItem;
      indexes[lastItem].Remove(lastIndex);
      indexes[lastItem].Add(valIndex);
    }
...
public class SolutionTests
{
  ...
  public void Test1()
  {
    ...
    randomizedCollection.GetRandom().Should().BeInRange(1, 2); // GetRandom should return 1 or 2, both equally likely.
  }
